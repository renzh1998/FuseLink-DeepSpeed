...
class DeepSpeedEngine(Module):
    ...
    def __init__(
        ...
        print("rank", torch.distributed.get_rank(), "configure distributed model")
        ...
        print("rank ", torch.distributed.get_rank(), "has optimizer", has_optimizer)
        if has_optimizer:
    ...
    def _broadcast_model(self):
        ...
        for p in self.module.parameters():
            ...
            if is_moe_param(p):
                ...
                if torch.is_tensor(p) and is_replicated(p):
                    tag += 1
                    print("[LOG] rank", torch.distributed.get_rank(), " tag", tag, ", before broadcasting group size", groups._get_sequence_data_parallel_world_size(), " root ", groups._get_broadcast_src_rank())
                    dist.broadcast(p, groups._get_broadcast_src_rank(), group=self.seq_data_parallel_group)
                    # dist.barrier()
                    print("[LOG] rank", torch.distributed.get_rank(), " tag", tag, ", after broadcasting")
    ...
    def _configure_distributed_model(self, model):
        ...
        if not (self.amp_enabled() or is_zero_init_model):
            print("[LOG] call _broadcast_model rank", torch.distributed.get_rank())
    ...
    def _configure_optimizer(self, client_optimizer, model_parameters):
        ...
        if optimizer_wrapper == ZERO_OPTIMIZATION:
            ...
            self._set_client_model(model)
            print("[LOG] before1 rank ", torch.distributed.get_rank())
            self._broadcast_model()
            print("[LOG] after1")
