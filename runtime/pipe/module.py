...
class PipelineModule(nn.Module):
    ...
    def __init__(self,
        ...
        print("rank", dist.get_rank(), "init pipeline module")
        super().__init__()
        ...
        print("rank ", dist.get_rank(), "run _synchronize_tied_weights")
    ...
    def _synchronize_tied_weights(self):
        print("rank", dist.get_rank(), "run _sync_tied_weights", self.tied_comms.items())
        for key, comm in self.tied_comms.items():
            for attr_name in comm['weight_attr']:
                print("brdcast rank", dist.get_rank(), "src", comm['ranks'])
                dist.broadcast(
                    getattr(comm['module'], attr_name),
                    src=min(comm['ranks']),
                    group=comm['group'],
                )
                print("brdcast rank", dist.get_rank(), "done")
        dist.barrier()
        print("rank", dist.get_rank(), "after barrier")
