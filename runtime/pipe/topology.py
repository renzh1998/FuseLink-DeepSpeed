...
class ProcessTopology:
    ...
    def __init__(self, axes, dims):
        ...
        tp = self.get_dim('model') if self.get_dim('model')!=0 else 1
        pp = self.get_dim('pipe')
        gpus_count = torch.cuda.device_count() # gpus in a node
        num_nodes = pp // (gpus_count // tp) # num of nodes need for pp
