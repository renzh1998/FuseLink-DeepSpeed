...
class PipelineEngine(DeepSpeedEngine):
    ...
    def __init__(self, has_bool_tensors=False, *super_args, **super_kwargs):
        print("rank ", torch.distributed.get_rank(), "init deepspeed engine")
        ...
        print("rank", dist.get_rank(), "in pipeengine")
        ...
        print("[LOG] rank", torch.distributed.get_rank(), ", before all reducing on group", self.grid.get_model_parallel_world_size())
        dist.barrier()
        dist.all_reduce(params_tensor, group=self.grid.get_model_parallel_group())
        dist.barrier()
        print("[LOG] rank", torch.distributed.get_rank(), ", after all reducing")
    ...
    def _build_data_iter(self, dataset):
        ...
        print("rank", dist.get_rank(), "build data sampler done")
    ...
    def _exec_reduce_tied_grads(self):
        ...
        for weight, group in weight_group_list:
            ...
            print("rank", dist.get_rank(), "before all reducing on exec_reduce_tied_grads all reduce")
            dist.all_reduce(grad, group=group)
            print("rank", dist.get_rank(), "after all reducing on exec_reduce_tied_grads all reduce")
        ...
        if reduce.lower() == 'avg':
            ...
            if reduce_dp and self.is_data_parallel:
                if torch.is_tensor(reduced):
                    print("rank", dist.get_rank(), "before all reducing on reduce_outputs all reduce")
                    dist.all_reduce(reduced, group=self.mpu.get_data_parallel_group())
                    print("rank", dist.get_rank(), "before all reducing on reduce_outputs all reduce")
                    reduced /= self.dp_world_size
                else:
                    for idx in range(len(reduced)):
                        print("rank", dist.get_rank(), "before all reducing on reduce_outputs else all reduce")

                        dist.all_reduce(reduced[idx], group=self.mpu.get_data_parallel_group())
                        print("rank", dist.get_rank(), "after all reducing on reduce_outputs else all reduce")
    ...
    def _bcast_pipe_scalar(self, data, src_rank=None, dtype=torch.float32):
        ...
        print("rank", dist.get_rank(), "before broadcasting on _bcast_pipe_scala")
        dist.broadcast(tensor=result, src=src_rank, group=self.mpu.get_pipe_parallel_group())
        print("rank", dist.get_rank(), "after broadcasting on _bcast_pipe_scala")
    ...
    def _aggregate_total_loss(self):
        ...
        if self.is_last_stage():
            ...
            if self.is_data_parallel:
                print("rank", dist.get_rank(), "before all reduce on aggregate total loss")
                dist.all_reduce(agg_loss, group=self.mpu.get_data_parallel_group())
                print("rank", dist.get_rank(), "after all reduce on aggregate total loss")
                agg_loss /= self.dp_world_size

            assert self.global_rank in self.grid.pp_group
            losses = torch.stack([self.dp_group_loss, agg_loss]).float()
            if self.is_pipe_parallel:
                print("rank", dist.get_rank(), "before bcast1 on aggregate total loss")
                dist.broadcast(tensor=losses, src=self.global_rank, group=self.mpu.get_pipe_parallel_group())
                print("rank", dist.get_rank(), "after bcast1 on aggregate total loss")
        else:
            # Get loss from last stage
            src_rank = self.grid.stage_to_global(self.num_stages - 1)
            assert src_rank in self.grid.pp_group
            losses = torch.Tensor([0., 0.]).to(self.device)
            print("rank", dist.get_rank(), "before bcast2 on aggregate total loss")
            dist.broadcast(tensor=losses, src=src_rank, group=self.grid.get_pipe_parallel_group())
            print("rank", dist.get_rank(), "after bcast2 on aggregate total loss")
